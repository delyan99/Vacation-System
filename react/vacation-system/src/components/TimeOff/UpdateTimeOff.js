...
const UpdateTimeOff = () => {
    ...
    const [status, setStatus] = useState(timeOffType === 'Sick Leave' ? 'Approved' : 'Pending');
                        ...
                        data.map(item => {
                            if (item.user.id === user.id && item.status === 'Approved') {
                                ...
                                while (startDate <= endDate) {
                                  console.log('Full date : ' + startDate);
    ...
    useEffect(() => {
        if (flag.current) {
            flag.current = false;
            getUserInfoByEmail();
            setTimeOffId(localStorage.getItem('timeOffId'));
            setStartDate(new Date(localStorage.getItem('timeOffStartDate')));
            setEndDate(new Date(localStorage.getItem('timeOffEndDate')));
            setDays(localStorage.getItem('timeOffDays'));
            setTimeOffType(localStorage.getItem('timeOffType'));
            setNote(localStorage.getItem('timeOffNote'));      
        }
    })
    ...
    return (
        <div>
            <div><NavBar /></div>
            <div className="timeOffUpdate">
                <h1>Time Off Update</h1>
                <form>
                    <div>
                        Start Date
                    </div>
                    ...
                    <label>
                        ...
                        <div>
                            <textarea value={note} onChange={(event) => setNote(event.target.value)}></textarea>
                        </div>
                    </label>
                    <br></br>
                    <div>
